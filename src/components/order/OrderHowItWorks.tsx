...
export default function OrderHowItWorks() {
  const { t } = useLanguage();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-flex items-center px-3 py-1 bg-accent-50 text-accent-500 text-xs font-medium rounded-full border border-accent-100">
            {t.orderHowItWorks.badge}
          </span>
          <h2 className="mt-4 text-3xl sm:text-4xl lg:text-5xl font-display font-bold text-gray-900 tracking-tight">
            {t.orderHowItWorks.title}
          </h2>
          <p className="mt-4 text-gray-500 text-lg">{t.orderHowItWorks.subtitle}</p>
        </motion.div>
        ...
        >
          {t.orderHowItWorks.steps.map((step, i) => (
            <motion.div key={i} variants={itemVariants} className="relative">
              {i < t.orderHowItWorks.steps.length - 1 && (
              ...
              <div className="relative text-center lg:text-left">
                ...
                <p className="mt-2 text-sm text-gray-500 leading-relaxed max-w-xs mx-auto lg:mx-0">{step.description}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
