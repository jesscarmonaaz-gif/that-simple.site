...
type HeroImages = {
  main: { src: string; alt: string; width: number; height: number; browserUrl: string };
  floating: FloatingMobile | FloatingBrowser;
};

const heroImages: Record<"booking" | "order", HeroImages> = {
  booking: {
    main: {
      ...
      width: 1920,
      height: 1080,
      browserUrl: "booking.that-simple.app/dashboard",
    },
    floating: {
      type: "mobile",
  ...
  order: {
    main: {
      ...
      width: 3008,
      height: 1454,
      browserUrl: "order.that-simple.app/dashboard",
    },
    floating: {
      type: "browser",
      ...
      width: 3002,
      height: 1444,
      browserUrl: "order.that-simple.app/pedidos",
    },
  },
};
...
export default function ProductHero({ product, ctaUrl }: ProductHeroProps) {
  const { t } = useLanguage();
  const hero = product === "booking" ? t.hero : t.orderHero;
  const { main, floating } = heroImages[product];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="relative">
              ...
              <div className="bg-gray-900 rounded-2xl p-2 shadow-2xl">
                <div className="flex items-center gap-2 mb-2 px-3 py-1.5">
                  ...
                  <div className="flex-1 bg-gray-700 rounded-full h-5 ml-3 flex items-center px-3">
                    <span className="text-[10px] text-gray-400">{main.browserUrl}</span>
                  </div>
                </div>
              ...
              {/* Floating element — mobile phone (Booking) */}
              ...
              {/* Floating element — browser window (Order) */}
              {floating.type === "browser" && (
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.6, delay: 0.8 }}
                  ...
                  <div className="bg-gray-900 rounded-xl p-1.5 shadow-2xl border border-gray-700">
                    <div className="flex items-center gap-1.5 mb-1 px-2 py-1">
                      ...
                      <div className="flex-1 bg-gray-700 rounded-full h-4 ml-2 flex items-center px-2">
                        <span className="text-[8px] text-gray-400">{floating.browserUrl}</span>
                      </div>
                    </div>
                    <div className="rounded-b-lg overflow-hidden">
                      <Image
